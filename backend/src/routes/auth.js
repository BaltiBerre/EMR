...
router.post('/register', [
  ...
  // checks for errors using the express-validator library
...
router.post('/create-user', async (req, res) => {
  ...
  try {
    // hashes the value in password using the bycrpt algorithm (10 rounds)
    const hashedPassword = await bcrypt.hash(password, 10);
    // inserts it into the database with a query
    const query = 'INSERT INTO UserAccounts (Username, PasswordHash, Role) VALUES ($1, $2, $3) RETURNING *';
    const values = [username, hashedPassword, role];
    // queries the database with the respective user details     
    const result = await pool.query(query, values);

    // if successful will return all the newly created users info
